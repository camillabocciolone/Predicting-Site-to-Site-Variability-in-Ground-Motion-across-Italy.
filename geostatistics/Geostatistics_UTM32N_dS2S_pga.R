...
#dS2S_pga ~sqrt(Vs30_Brunelli_simple) + 1/slope50_simple
f = dS2S_SA_pga ~ 1/slope50_simple + sqrt(Vs30_Brunelli_simple)
ita = itaca[!is.na(itaca$slope50_simple) & !is.na(itaca$Vs30_Brunelli_simple) &!is.na(itaca$dS2S_SA_pga), ] #take the initial datframe for simplicity (in order to remove only necessary NA). OSS. Don't remove NA of x,y, there are not
...
ggplot(lz.sf) +
  ...
  theme_minimal() +
  labs(color = "Predicted Value", title = "Universal Kriging Prediction: dS2S for T=0s")
#map for the var
ggplot(lz.sf) +
  geom_sf(aes(color = var1.var)) +  
  scale_color_viridis_c(na.value = "grey90") +
  theme_minimal() +
  labs(color = "Predicted Value", title = "Universal Kriging Prediction: variability of dS2S for T=0s")
...
#4) dS2S_pga ~sqrt(Vs30_Brunelli_simple) + 1/slope50_simple + LitoBucci
itaca$Lito_Bucci_simple=factor(itaca$Lito_Bucci_simple)
levels(itaca$Lito_Bucci_simple)
f = dS2S_SA_pga ~ 1/slope50_simple + sqrt(Vs30_Brunelli_simple) + Lito_Bucci_simple
ita = itaca[!is.na(itaca$slope50_simple) & !is.na(itaca$Vs30_Brunelli_simple) &!is.na(itaca$dS2S_SA_pga) & !is.na(itaca$Lito_Bucci_simple), ] 
coordinates(ita)=c('x','y') #Create a spatial dataframe
proj4string(ita) = CRS("+proj=utm +zone=32 +datum=WGS84 +units=m +no_defs") #set CRS
...
#5) dS2S_pga ~sqrt(Vs30_Brunelli_simple) + 1/slope50_simple + LitoBucciGrouped
itaca$Lito_Bucci_grouped=factor(itaca$Lito_Bucci_grouped)
levels(itaca$Lito_Bucci_grouped)
f = dS2S_SA_pga ~ 1/slope50_simple + sqrt(Vs30_Brunelli_simple) + Lito_Bucci_grouped
ita = itaca[!is.na(itaca$slope50_simple) & !is.na(itaca$Vs30_Brunelli_simple) &!is.na(itaca$dS2S_SA_pga) & !is.na(itaca$Lito_Bucci_grouped), ] 
coordinates(ita)=c('x','y') #Create a spatial dataframe
proj4string(ita) = CRS("+proj=utm +zone=32 +datum=WGS84 +units=m +no_defs") #set CRS
